"""
test functions, not using sequential, single layer, try to extrac weights
"""
import copy
...
# ------------- torch original -----------
class MechanicalSystem(nn.Module):  # on original, single layer

    def __init__(self, dt, n_x=2, init_small=True):
        super(MechanicalSystem, self).__init__()
        self.dt = dt  # sampling time
        self.hidden = 64
        self.layer1 = nn.Linear(n_x + 1, self.hidden)
        self.layer2 = nn.Linear(self.hidden, 1)
    ...
    def forward(self, x1, u1):
        ...
        out1 = self.layer1(in_xu)
        out2 = nn.ReLU(out1)
        out3 = self.layer2(out2)

        dv = out3 / self.dt  # v, dv = net(x, v)
...
class ForwardEulerEvolution(nn.Module):
    ...
    def __init__(self, model, dt, N, update=True, epoch=100, threshold1=0, threshold2=0, slot=1000, lr=0.0001):
    ...
    def forward(self, x0: torch.Tensor, u: torch.Tensor, y: torch.tensor):
        ...
        while q < self.N:
            ...
            dx = self.model(x_step, u_step)
            x_step = x_step + dx * self.dt
            self.xhat_data[q, :] = x_step[0, :]
            # E = y_step - x_step[0, [0]].clone().detach()  # numpy
            # sqE = E ** 2
            # VN0 = VN0 + sqE
            # VN = VN0 / q
            # self.VN_data[q] = VN
            if self.update:
                self.match = R2(y[q - self.slot:q], self.xhat_data[q - self.slot:q, 0])
                if self.match < 0.9:  # update
                    """
                   
                # slot_vn = self.VN_data[q - self.slot:q]
                # if np.var(slot_vn) > self.threshold2:
                # if VN > self.threshold2: """
                    self.evolute.append(q)
                    load = self.model.net.parameters
                    # load = self.model.state_dict()
                    model2 = Retrain(dt=self.dt)  #, load=load
                    model2.parameters = self.model.parameters
                    ...
                    optimizer = torch.optim.Adam(model2.parameters(), lr=self.lr)
                    ...
                    for epoch in range(self.epoch):
                        ...
                        for u_train_step in u_train.split(1):
                            ...
                            dx = model2(x_train, u_train_step)
                        ...
                        loss = torch.mean((error_out / 0.1) ** 2)
                        if (epoch + 1) % 100 == 0:
                            print(f'epoch {epoch + 1}/{self.epoch}: loss= {loss.item():.5f}, yhat= {yhat[:, 0]:.4f}')
                        optimizer.zero_grad()
                        loss.backward(retain_graph=True)  #
                        optimizer.step()

                    self.model.parameters = model2.parameters
                    q = q + 1
                    u_step = u[q]
                    y_step = y[q]
                    dx = self.model(x_step, u_step)
                    ...
                    if self.match > 0.9:
                        # E = y_step - x_step[0, [0]].clone().detach()
                        # sqE = E ** 2
                        # VN0 = VN0 + sqE
                        # VN = VN0 / q
                        # self.VN_data[q] = VN
                        # slot_vn = self.VN_data[q - self.slot:q]
                        # # if np.var(slot_vn) < self.threshold1:
                        # if VN < self.threshold1:
                        self.stop.append(q)
                        break
                    # q = q + 1
