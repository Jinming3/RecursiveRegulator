...
class PEM(object):  #
    ...
    def pem_ss(self, Y_sys, U, train=10000, step=1):  # can't be used embedded, has to be outside of any method, size-N !!!
        """
        not finished, state space. no K
        """
    ...
    def pem_step(self, Y_sys, U, step=1):  # can't be used embedded, has to be outside of any method, size-N !!!
        """
        on-off PEM, threshold and slot in parent function!!! when rest, no reading y
        similar to forward pem
        ...
        while q < self.N:
            ...
            if q > self.train:  # check if to stop # only KF-predictor no updating
                # self.Khat = self.Khat *0
                self.Y[:] = Y_sys[q]
                fix = self.Y - self.Yhat

                Xhat_new = np.dot(self.Ahat, self.Xhat) + self.Khat * fix + np.dot(self.Bhat, U[q])
