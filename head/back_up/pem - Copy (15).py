...
class PEM(object):  #
    ...
    # ------<<<<<<<<<< test <<<< ------------------------
    def ahead(self, Y_sys, U, step):# predict n step ahead,
        """
        Y_sys = N * 1
        U = N * 1
        """
        ...
        while q < self.N:
            ...
            self.Thehat = self.Thehat_old + np.dot(np.dot(P_old, Psi_old), self.E)  # (self.Y - self.Yhat)
            ...
            Xhat_new = np.dot(self.Ahat, self.Xhat) + self.Bhat * U[q] + self.Khat * (self.E)# (self.Y - self.Yhat)
            Yhat_new = np.dot(self.Chat, Xhat_new)
            predict = []
            while z < step:
                predict.append(np.dot(self.Chat, Xhat_new))
                Xhat = np.copy(Xhat_new)
                Xhat_new = np.dot(self.Ahat, Xhat) + self.Bhat * U[q+z+1]


            self.E = np.mse(Y_sys[q:q+step], predict)
            # update every parameter which is time-variant
            self.Xhat_old = np.copy(self.Xhat)
            self.Xhat = np.copy(Xhat_new)
            self.Xhat_data[q, :] = np.copy(self.Xhat[:, 0])
    ...
    def pem_ahead(self, Y_sys, U, step=5, on=True):
        """
        Y_sys = N*1
        U = N*1
        """
        ...
        if not on:  # check if to stop # only KF-predictor no updating
            ...
            # print(fix)
            # Xhat_new = np.dot(self.Ahat, self.Xhat)
            + self.Khat * self.fix  # (self.Y - self.Yhat)  #
            Yhat_new = np.dot(self.Chat, Xhat_new)
            self.Xhat_old = np.copy(self.Xhat)
            self.Xhat = np.copy(Xhat_new)
            self.U_old = np.copy(U)
