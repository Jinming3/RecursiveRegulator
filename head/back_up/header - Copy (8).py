...
class StepSystem(nn.Module):

    def __init__(self, dt, n_x=2, n_u=2, init_small=True):
        super(StepSystem, self).__init__()
        self.dt = dt  # sampling time
        self.hidden = 64 #100# is good
...
class NeuralStateSpaceModel(nn.Module):  # if not pos and vel, no derivative relation
    n_x: Final[int]
    n_u: Final[int]
    n_feat: Final[int]
...
class ForwardEulerPEM(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, u: torch.Tensor, y):
        ...
        while q < self.N:
            ...
            if self.update == 3:  # case 1 with threshold
                u_step = u[q]
                dx = self.model(x_step, u_step)
                x_step = x_step + dx * self.dt
                self.xhat_data[q, :] = x_step[0, :].clone().detach().numpy()  # collect output of NN
            ...
            if self.update == 5:  # case 4 with threshold, not adding resting PEM
                u_step = u[q]
                dx = self.model(x_step, u_step)
                x_step = x_step + dx * self.dt
