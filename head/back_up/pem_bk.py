"""
single-output pem of canonical form, may not apply to batch training
Functions:
peml--packet-loss situation
pemt--with threshold to stop and restart, when thres=0, nonstop PEM
...
class PEM(object):
    def __init__(self, n, t, N):
        ...
        self.k = 0
        self.VN0 = 0
        # for on-off PEM collecting zeitpunkte
        self.stop = []
        self.restart = []
        #  for PKF
        self.A = np.eye(n)
    ...
    def peml(self, Y_sys, U):
        ...
        for q in range(self.N):
            ...
            Xhatdot = self.Xhatdot0 + np.dot(self.Ahat_old, self.Xhatdot_old) - gamma_old * np.dot(self.Khat_old, self.Psi_old2.T)
            ...
            Xhat_new = np.dot(self.Ahat, self.Xhat) + np.dot(self.Bhat, gamma * U[q]) + gamma * np.dot(self.Khat, (s - self.Yhat))
