...
class BeerSystem(nn.Module):
    ...
    def __init__(self, n_in=2, n_params=4, init_small=True):
        super(BeerSystem, self).__init__()
        self.hidden = 64

        self.net = nn.Sequential(nn.Linear(n_in, self.hidden),  # x + u # use this
                                 # nn.LeakyReLU(negative_slope=0.4),
                                 nn.ReLU(),
                                 nn.Linear(self.hidden, n_params),
                                 # nn.ReLU(), nn.Linear(20, n_params)
                                 )
    ...
    def forward(self, y0, u0):

        init = torch.cat((y0, u0), -1)
        # init = y0
...
class KoopmanExample(nn.Module):  # original

    def __init__(self, dt, n_x=2, init_small=True):
        super(KoopmanExample, self).__init__()
        self.dt = dt  # sampling time
        self.hidden = 64
        self.net = nn.Sequential(nn.Linear(n_x , self.hidden),  # 3*1
                                 # nn.LeakyReLU(negative_slope=0.4),
                                 nn.ReLU(),
    ...
    def forward(self, x1):
        ...
        dv = self.net(x1) / self.dt  # v, dv = net(x, v)
        list_dx = [x1[..., [1]], dv]  # [dot x=v, dot v = a]
        dx = torch.cat(list_dx, -1)
        return dx


class ForwardEuler1(nn.Module):  # original

    def __init__(self, model, dt=1.0):
        super(ForwardEuler1, self).__init__()
        self.model = model
        self.dt = dt


    def forward(self, x0: torch.Tensor, batch=32) -> torch.Tensor:
        xhat_list = list()
        x_step = x0
        for i in np.arange(batch):
            dx = self.model(x_step)
...
class MechanicalSystem_mimo(nn.Module):  # original
...
class ForwardEulerPEM(nn.Module):  # use steps or R2 as switch
    ...
    def __init__(self, model,
                 ...
                 sensitivity=100, step=1, train=10000, param=np.array):
    ...
    def forward(self, x0: torch.Tensor, u: torch.Tensor, y):
        ...
        while q < self.N:
            ...
            if self.update == 3:
                ...
                match = R2(y[q - self.sensitivity:q, 0, 0], self.xhat_data[q - self.sensitivity:q, 0])
                q = q + 1
            # update with threshold, not adding resting PEM # use this!!!!
            if self.update == 5:
                u_step = u[q]
                dx = self.model(x_step, u_step)
                x_step = x_step + dx * self.dt
                ...
                if match < self.threshold1:
                    ...
                    while q < self.N:
                        ...
                        q = q + 1
                q = q + 1
            # test use threshold, resi added, better explained than  5
            if self.update == 7: #move pem to (dx+pem)*dt
                u_step = u[q]
                dx = self.model(x_step, u_step) + self.resi
                # x_step = x_step + dx * self.dt + self.resi
                x_step = x_step + dx * self.dt
                self.xhat_data[q, :] = x_step[0, :].clone().detach().numpy()  # collect
                # --------------------------------------------------------
                ...
                if match < self.threshold1:
                    ...
                    while q < self.N:
                        ...
                        if match > self.threshold2:
                            # self.resi[0, 0] = self.factor.Xhat[0, 0]
                            # self.resi[0, 1] = self.factor.Xhat[1, 0]
                            self.resi = torch.tensor(self.factor.pem_one(y[q] - y_nn, u_in, on=False).T, dtype=torch.float32)
            ...
            if self.update == 8: #move pem to (dx+pem)*dt
                ...
                if match < self.threshold1:
                    ...
                    while q < self.N:
                        ...
                        y_nn = x_step[:, 0].clone().detach().numpy()
                        u_in = y_nn
                        self.factor.ahead(y[q] - y_nn, u_in, step=self.step)
                        ...
                        if match > self.threshold2:
                            # self.resi[0, 0] = self.factor.Xhat[0, 0]
                            # self.resi[0, 1] = self.factor.Xhat[1, 0]
                            self.resi = torch.tensor(self.factor.ahead(y[q] - y_nn, u_in, step=self.step), dtype=torch.float32)  #.T
