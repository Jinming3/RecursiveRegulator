        ...
        while q < self.N:
            ...
            if self.update == 4: #move pem to (dx+pem)*dt, copied from 7, use self.train to stop PEM, not R2
                ...
                while q < self.train:
                        ...
                        q = q + 1
                q = q + 1
            if self.update == 8: # xtep = x_step+pem, outside of dx, copied from 5, use self.train to stop PEM, not R2, not added when stop
                u_step = u[q]
                # print("alter=", self.alter)
                dx = self.model(x_step, u_step)#+ self.alter
                #
                x_step = x_step + dx * self.dt + torch.tensor(self.factor.Xhat[:, 0], dtype=torch.float32)#+ self.alter
