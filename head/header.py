...
####
class MechanicalSystem_u(nn.Module):  # koopman
...
class NeuralStateSpaceModel_u(nn.Module):  # when not pos and vel, no derivative relation
    n_x: Final[int]
    n_u: Final[int]
    n_feat: Final[int]

    def __init__(self, n_x=2, n_u=1, n_feat=64, scale_dx=1.0, init_small=True, activation='relu'):
        super(NeuralStateSpaceModel_u, self).__init__()
        self.n_x = n_x
        self.n_u = n_u
        self.n_feat = n_feat
    ...
    def forward(self, in_x, in_u):
        ...
        self.out_k = self.phi_k(in_x)
        self.out_b = self.phi_b(in_u)

        self.out_q = self.out_k + self.out_b

        dx = self.inv_phi(self.out_q)
    ...
    def forward(self, x0: torch.Tensor, u: torch.Tensor, y):
        ...
        while q < self.N:
            ...
            if self.update == 120:  # case 121, stop pem update at given time, self.train, u_in = 64, experiment of pem matrix
                ...
                x_step = x_step + dx * self.dt+ torch.tensor(self.factor.Xhat[:, 0], dtype=torch.float32)*ignition_true
                self.err[q] = y[q]-x_step[0, 0].clone().detach().numpy()
                # x_step = x_step
                self.xhat_data[q, :] = x_step[0, :].clone().detach().numpy()  # collect output of NN
