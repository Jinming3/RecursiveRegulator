...
num_epoch = 50  # in training
model_filename = f"{system}_batch{batch}_epo{num_epoch}"
...
# df1 = pd.concat([df1, pd.DataFrame(time_exp, columns=['time'])], axis=1)
# df1 = pd.concat([df1, pd.DataFrame(online.Yhat_data, columns=['filter'])], axis=1)
# df1.to_csv(f'results_{system}_infer.csv', index=False)
# # -----------CED -----------
