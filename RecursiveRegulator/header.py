...
class MechanicalSystem_qu(nn.Module):  # koopman NN
    ...
    def __init__(self, dt, n_x=2, init_small=True):
        ...
        self.inv_phi = nn.Linear(self.hidden, 1, bias=False)  # reverse funtion, back to original space
...
class ForwardEulerPEM(nn.Module):  
    ...
    def __init__(self, model,factor, dt, N, update, threshold1=1, threshold2=1,
        ...
        self.N = N
        n=self.factor.n
        self.update = update  # choose case, 12010 for Recursive Regulator
        if train == 0:
            self.train = int(N)
        else:
    ...
    def forward(self, x0: torch.Tensor, u: torch.Tensor, y):
        ...
        while q < self.N:
            # works, in use---->>>>>>>
            if self.update == 12010:  # in paper! u_in = 65, regulator G q and u
                u_step = u[q]
                dx = self.model(x_step, u_step)
