...
Functions:
pem_one -- dependent function, for Koopman NN
...
class PEM(object):  
    ...
    def pem_step(self, Y_sys, U, train=10000,step=1):  # can't be used embedded, has to be outside of any method, size-N !
        """
        ...
        while q < self.N:
            ...
            if q > self.train:  # check if to stop # only KF-predictor no updating
                ...
                Xhat_new = np.dot(self.Ahat, self.Xhat) + np.dot(self.Bhat, U[q]) 
    ...
    # ------<<<<<<<<<< test <<<< ------------------------
    def ahead(self, Y_sys, U, step, pred_ahead=True):# predict n step ahead,
        """
        Y_sys = N * 1
        U = N * 1
        """
        ...
        while q < self.N:
            ...
            self.Thehat = self.Thehat_old + np.dot(np.dot(P_old, Psi_old), (self.Y - self.Yhat))  # self.E
            ...
            Xhat_new = np.dot(self.Ahat, self.Xhat) + self.Bhat * U[q] + self.Khat * (self.Y - self.Yhat)# (self.Y - self.Yhat)
            Yhat_new = np.dot(self.Chat, Xhat_new)
            predict = []
            Xhat_step_new = np.copy(Xhat_new)

            if 1000 < q < self.N-step:
                if pred_ahead:

                    for z in range(step):
                        predict.append(np.dot(self.Chat, Xhat_step_new))
                        Xhat_step = np.copy(Xhat_step_new)
                        m1= np.dot(self.Ahat, Xhat_step)
                        m2= self.Bhat * U[q+z+1]  # if pem use n step ahead, nn should also, and give pem all u and y

                        Xhat_step_new = m1 +m2  #+ self.Khat * (self.E)

                    self.E = mse(Y_sys[q:q + step], predict)
                    print('E=', self.E)
            ...
            q = q + 1
        return self.Yhat_data#, self.E_data
